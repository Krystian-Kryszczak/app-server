...
import app.server.service.email.EmailService;
import app.server.service.email.configuration.EmailConfiguration;
...
public class AccountServiceImpl implements AccountService {
    ...
    @Inject
    EmailConfiguration.ForgotPassword forgotPasswordConfig;
    ...
    @Override
    public Publisher<HttpStatus> forgotPassword(@Email String email) {
        return Mono.from(userRepository.findDocumentByEmail(email)).flatMap(doc -> {
            ...
            return resultPublisher.map(result -> {
                ...
                boolean success = emailService.sendEmail(email, forgotPasswordConfig.getSubject(),
                    CollectionUtils.mapOf("name", doc.getString("name"), "code", code, "appName", appName),
                    forgotPasswordConfig.getTemplate());
                if (value!=null&&success) return HttpStatus.OK;
                else return HttpStatus.CONFLICT;
            });
        }).defaultIfEmpty(HttpStatus.CONFLICT);
    }
    @Override
    public Publisher<HttpResponse<HttpStatus>> resetPassword(@NonNull String code, @NotNull String newPassword) {
                ...
                monoSink -> {
                    ...
                    if (ZonedDateTime.of(LocalDateTime.now(), ZoneId.of(instanceConfiguration.getZone().orElse("Europe/Warsaw")))
                        ...
                        Mono.from(userRepository.findById(resetCode.getUserId())).flatMap(
                                user -> Mono.from(userRepository.changePassword(resetCode.getUserId(), passwordEncoder.encode(newPassword))) // if user has been found in database
                                        .flatMap(
